var map = ui.Map();
map.style().set('cursor', 'crosshair'); // Change cursor style for point selection

// Create a panel to hold the charts and text outputs
var outputPanel = ui.Panel({
  style: {width: '400px', padding: '8px'}
});
outputPanel.add(ui.Label('Click on the map to select an area of interest'));

// Set up the layout
ui.root.clear();
ui.root.setLayout(ui.Panel.Layout.Flow('horizontal'));
ui.root.add(map);
ui.root.add(outputPanel);

// Function to update the UI based on the selected point
function updateUI(point) {
  outputPanel.clear(); // Clear previous outputs
}
// Define a region of interest as a point
var pointOfInterest = ee.Geometry.Point(-105.077209, 39.766098);
...
function printTotalPrecipitation(image, label) {
 ...
 totalPrecipitation.evaluate(function(result) {
  var labelText = label + ': ' + (result || 0).toFixed(2) + ' mm';
  outputPanel.add(ui.Label(labelText));
});
}
...
var bufferedRegion = pointOfInterest.buffer(10000); // Buffer by 10 km to capture surrounding fields
...
totalCropArea.evaluate(function(result) {
  var labelText = label + ' Area (hectares) - ' + colorHex + ': ' + result;
  outputPanel.add(ui.Label(labelText));
});
}
...
meanSurfaceSoilMoisture.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: pointOfInterest,
  scale: 10000
}).get('sm_surface').evaluate(function(value) {
  var labelText = 'Surface Soil Moisture Value (b/w 0.2-0.3): ' + value;
  outputPanel.add(ui.Label(labelText));
});

meanRootZoneSoilMoisture.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: pointOfInterest,
  scale: 10000
}).get('sm_rootzone').evaluate(function(value) {
  var labelText = 'Root Zone Soil Moisture Value (b/w 0.2-0.3): ' + value;
  outputPanel.add(ui.Label(labelText));
});
